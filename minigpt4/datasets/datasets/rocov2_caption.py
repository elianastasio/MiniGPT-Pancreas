...
from minigpt4.datasets.datasets.caption_datasets import ROCOv2CaptionTrainDataset, ROCOv2CaptionEvalDataset, CaptionEvalDataset

ROCOv2CapTrainDataset = ROCOv2CaptionTrainDataset
ROCOv2CapEvalDataset = ROCOv2CaptionEvalDataset
...
class RefROCOEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_id = data['img_id']
        sent = data['sents']
        image_path = os.path.join(self.root_path, f'{img_id[:10]}.jpg') #messo a 10 da me
        image = Image.open(image_path).convert('RGB')
        image = self.vis_processor(image)
