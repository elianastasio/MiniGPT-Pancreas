...
class TCEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_name = data['image_path']#elia: da img_id a qid
        question = "does the pancreas contain a tumor?" #data['question']
        qid = data['q_id']
        #img_file = '{}.jpg'.format(img_id) #elia: commentato
        image_path = os.path.join(self.root_path, img_name)#elia: img_file to img_id
        image = Image.open(image_path).convert('RGB')
        image = self.vis_processor(image)
        question = f"[vqa] Based on the image, respond to this question with a short answer: {question}"
        return image, question, qid, img_name
...
class VQARADEvalData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        data = self.loaded_data[idx]
        #print("Elia: Keys in data:", data.keys())#elia
        img_name = data['image_name']#elia: da img_id a qid
