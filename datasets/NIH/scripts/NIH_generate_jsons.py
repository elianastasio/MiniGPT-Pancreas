...
def process_datasets(balanced=False):
    ...
    data = load_json(os.path.join(output_dir, "NIH_slices_info.json"))
    ...
    train_path = os.path.join(output_dir, f"NIH_{'balanced_' if balanced else ''}train.json")
    test_path = os.path.join(output_dir, f"NIH_{'balanced_' if balanced else ''}test.json")
